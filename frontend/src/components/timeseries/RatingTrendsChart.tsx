...
const LINE_COLORS = ['#8b5cf6', '#f97316', '#2ea597', '#c33d69'];
...
const RatingTrendsChart: React.FC<RatingTrendsChartProps> = ({
  ...
  const { chartData, seriesNames, ratingTrend, competitivePosition, displayReviewCount } = useMemo(() => {
    if (!ratingsData?.data || ratingsData.data.length === 0) {
      return { chartData: [], seriesNames: [], ratingTrend: null, competitivePosition: null, displayReviewCount: 0 };
    }

    // Collect all unique periods
    const allPeriods = new Set<string>();
    ratingsData.data.forEach((p) => allPeriods.add(p.period_start));
    comparisonRatingsSource?.data?.forEach((p) => allPeriods.add(p.period_start));
    ...
    const sortedPeriods = Array.from(allPeriods).sort((a, b) => getDateSortKey(a) - getDateSortKey(b));
    ...
    if (categoryRatingsData?.data && selectedCategory) {
      names.push(`${selectedCategory} in ${selectedCity}`);
    }
    ...
    const data: ChartDataPoint[] = sortedPeriods.map((periodStart) => {
      ...
      // Add category data
      if (categoryRatingsData?.data && selectedCategory) {
        point[`${selectedCategory} in ${selectedCity}`] = categoryPoint?.avg_rating || 0;
      }
    ...
    const totalReviews = ratingsData.data.reduce((sum, point) => sum + (point.review_count || 0), 0);
    ...
    return {
      ...
      displayReviewCount: totalReviews > 0 ? totalReviews : business?.review_count || 0,
    };
  }, [
    ratingsData,
  ...
  return (
    <div className="rating-trends-chart" style={{ backgroundColor: BACKGROUND_COLOR }}>
      ...
      <div className="rating-trends-chart__header">
        ...
        <div className="rating-trends-chart__subtitle">
          {business ? (
            <>
              <span>{business.city}, {business.state}</span>
              <span className="rating-trends-chart__dot">•</span>
              <span className="rating-trends-chart__stars">
                <Star size={12} fill="white" strokeWidth={0} /> {business.stars}
              </span>
              <span>({displayReviewCount.toLocaleString()} reviews)</span>
            </>
          ) : selectedNeighborhood ? (
            <span>
              {selectedNeighborhood
